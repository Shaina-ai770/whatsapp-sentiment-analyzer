...
def apply_multilingual_analysis(df, show_progress=True, include_lang_col=True):
    ...
    progress_bar = st.progress(0, text="🌍 Analyzing sentiment...") if show_progress else None
    ...
    for i, text in enumerate(df["message"].fillna("").astype(str).tolist()):
        ...
        if progress_bar and i % 5 == 0:
            ...
            progress_bar.progress(pct,
                text=f"{flag} {i+1}/{total} — {lang.capitalize()}")
...
def render_vader_dashboard(df, sentiment_col):
    ...
    (chat_tab, overview_tab, url_tab, sentiment_tab, users_tab,
     words_tab, activity_tab, top_tab, all_tab) = st.tabs([
        "📝 Summary", "📊 Overview", "🔗 URLs", "😊 Sentiment",
        "👥 Users", "☁️ Words", "📅 Activity", "🏆 Top", "💬 Messages"
    ])
        ...
        for i, r in _adf.iterrows():
            ...
            for c, v in zip(_adf.columns, r.values):
                if c == sentiment_col:
                    ...
                    cells += f"<td style='padding:8px 12px;color:{sc};font-weight:700;font-size:0.82rem;border-bottom:1px solid {_br};'>{v}</td>"
                else:
...
if "raw_chat_bytes" in st.session_state:
    ...
    with st.sidebar:
        ...
        if "Transformers" in analysis_method:
            lang_opts = ["English"] + [l.capitalize() for l in SUPPORTED_LANGUAGES if l != "english"]
            st.selectbox("Language", options=lang_opts, index=0,
                         key="transformer_language_selector", on_change=st.rerun)
    ...
    with col1: ui_metric_card("Total Messages", f"{len(filtered_df):,}", "💬")
    with col2: ui_metric_card("Positive", f"{len(filtered_df[filtered_df[sentiment_col].str.lower()=='positive']):,}", "✅")
    with col3: ui_metric_card("Negative", f"{len(filtered_df[filtered_df[sentiment_col].str.lower()=='negative']):,}", "❌")
    with col4: ui_metric_card("Active Users", f"{filtered_df['author'].nunique():,}", "👥")
    ui_divider()
    ...
    # ── Transformers Dashboard ──
        ...
        tabs = st.tabs(["📝 Summary", "📊 Overview", "🔗 URLs", "😊 Sentiment",
                         ...
                         "🔄 Comparison", "🧠 Behavioral", "🤖 Model", "💬 Messages"])
        (chat_tab, overview_tab, url_tab, sentiment_tab, emotion_tab, words_tab,
         conf_tab, comparison_tab, behavioral_tab, model_tab, all_messages_tab) = tabs
    ...
    # ── Multimodal Dashboard ──
        ...
        (ov_tab, emoji_tab, fusion_tab, sent_tab, em_tab, users_tab, words_tab,
         act_tab, top_tab, all_tab, model_tab) = st.tabs([
            "📈 Overview", "🎭 Emoji Analysis", "🔀 Text+Emoji Fusion",
            "😊 Sentiment", "😊 Emotions", "👥 Users",
            "☁️ Words", "📅 Activity", "🏆 Top", "💬 Messages", "🤖 Model"
        ])
